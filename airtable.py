    ...
    data = {
        "records": [{
            "fields": {
                ...
                "Thread ID": (thread_id_match.group(1) if (thread_id_match := re.search(r"thread_id='?([^'\s]*)'?", response_data['body'])) else "N/A") if isinstance(response_data['body'], str) else "N/A",
