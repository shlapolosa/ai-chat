    ...
    data = {
        "records": [{
            "fields": {
                ...
                "Thread ID": request_data_serializable.get('request', {}).get('thread_id', (thread_id_match.group(1) if (thread_id_match := re.search(r"thread_id='?([^'\s]*)'?", response_data['body'])) else "N/A")) if isinstance(response_data['body'], str) else "N/A",
                "Run ID": (run_id_match.group(1) if (run_id_match := re.search(r"run_id='?([^'\s]*)'?", response_data['body'])) else "N/A") if isinstance(response_data['body'], str) else "N/A",
