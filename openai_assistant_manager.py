            ...
            if config.get("active", True):
                if config.get("assistant_id"):
                    ...
                    assistant_creation_params = {
                        ...
                        "tools": [self.generate_tool_configurations(getattr(gpt_tools, tool["name"])) if tool["type"] == "function" and hasattr(gpt_tools, tool["name"]) else {"type": tool["type"]} for tool in config.get("tools", [])],
